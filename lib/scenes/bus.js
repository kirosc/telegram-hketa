...
async function checkCircular(ctx, company, route) {
    ...
    switch (company) {
        ...
        case 'MTR':
            ...
            if (res.route.lines.length > 1) {
                await askDirection(ctx, company, route, { lines: res.route.lines });
            } else {
                await askStops(ctx, company, route, { routeId: res.route.lines[0].id });
            }
            break;
    }
}
...
async function askStops(ctx, company, route, options) {
    ...
    switch (company) {
        ...
        case 'MTR':
            ...
            const { line } = await getMTRStops(routeId);
            for (const { name_ch, ref_ID } of line.stops) {
